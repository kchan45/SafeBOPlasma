...
# filepath: ax/modelbridge/torch.py
...
class TorchModelBridge(ModelBridge):
    ...
    def _gen(
        ...
        optimization_config: Optional[OptimizationConfig] = None,
        ##################### BEGIN EDITS BY K.J. CHAN #####################
        safe_set: Optional[List[ObservationFeatures]] = None,
        ###################### END EDITS BY K.J. CHAN ######################
    ) -> GenResults:
        ...
        if safe_set is None:
            safe_set = []
        safe_set_tensor = self._transform_observation_features(safe_set)
        # print("transformed safe observation features to tensor")
        ###################### END EDITS BY K.J. CHAN ######################
        ...
        ##################### BEGIN EDITS BY K.J. CHAN #####################
        # print("converting safe candidates to ObservationFeatures")
        gen_metadata["safe_cand_as_of"] = self._array_to_observation_features(
            X=gen_metadata["safe_candidates"],
            candidate_metadata=gen_results.candidate_metadata,
        )
        ###################### END EDITS BY K.J. CHAN ######################
