...
# filepath: ax/core/experiment.py
...
class Experiment(Base):
    ...
    def __init__(
        ...
        ##################### BEGIN EDITS BY K.J. CHAN #####################
        self._safe_set: List[Any] = []
        self._eaf_trace: List[Any] = []
        ###################### END EDITS BY K.J. CHAN ######################

    ##################### BEGIN EDITS BY K.J. CHAN #####################
    @property
    def safe_set(self) -> List[Any]:
        """Return the experiment's safe set of observation features"""
    ...
    def new_trial(
        ...
        ##################### BEGIN EDITS BY K.J. CHAN #####################
        print("saving safe set to experiment")
        if (generator_run is not None):
            if ("safe_cand_as_of" in generator_run.gen_metadata.keys()):
                self._safe_set = generator_run.gen_metadata["safe_cand_as_of"]
            if ("prev_expected_acq_value" in generator_run.gen_metadata.keys()):
                self._eaf_trace.append(generator_run.gen_metadata["prev_expected_acq_value"])
