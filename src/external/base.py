...
# filepath: ax/modelbridge/base.py
...
class ModelBridge(ABC):
    ...
    def __init__(
        ...
        ##################### BEGIN EDITS BY K.J. CHAN #####################
        self._safe_set: Optional[List[ObservationFeatures]] = []
        # extract safe set from experiment
        safe_obsf = self._prepare_safe_set(experiment=experiment)
        # set the attribute for this object
        safe_obsf_raw = self._set_safe_set(obsf=safe_obsf)
        safe_obsf = self._transform_safe_obsf(
            obsf=safe_obsf_raw,
            observations=observations_raw,
            search_space=search_space,
            transforms=transforms,
        ...
        # print(safe_obsf)
        # print("ModelBridge has new attribute _safe_set and has been successfully set.")
        self._eaf_trace = experiment._eaf_trace
        ###################### END EDITS BY K.J. CHAN ######################
    ...
    ##################### BEGIN EDITS BY K.J. CHAN #####################
    def _prepare_safe_set(
        self, experiment: Optional[Experiment]
    ) -> List[ObservationFeatures]:
        # print("preparing safe_set from experiment")
        if experiment is None:
            return []
        return experiment.safe_set
    ###################### END EDITS BY K.J. CHAN ######################
    ...
    def _transform_safe_obsf(
        ...
        """Apply transforms to safe set observation features."""
        # print("applying transforms to safe set")
        search_space = search_space.clone()
        if transforms is not None:
            if transform_configs is None:
                transform_configs = {}
            ...
            for t in transforms:
                ...
                obsf = t_instance.transform_observation_features(obsf)
        return obsf
    ###################### END EDITS BY K.J. CHAN ######################
    ...
    ##################### BEGIN EDITS BY K.J. CHAN #####################
    def _set_safe_set(
        self, obsf: List[ObservationFeatures]
    ) -> List[ObservationFeatures]:
        """Store safe set, not-transformed."""
        # print("storing safe set as attribute")
        self._safe_set = deepcopy(obsf)
        return obsf
    ###################### END EDITS BY K.J. CHAN ######################
    ...
    def gen(
        ...
        if isinstance(self.model, TorchModel):
            safe_obsf = self._transform_safe_obsf(
                ...
                search_space=orig_search_space.clone(),
                transforms=self._raw_transforms,
                transform_configs=self._transform_configs,
            )
            if model_gen_options is None:
                model_gen_options = {}
            model_gen_options["eaf_trace"] = self._eaf_trace
        ###################### END EDITS BY K.J. CHAN ######################
        ...
        for t in reversed(list(self.transforms.values())):
            ...
            ##################### BEGIN EDITS BY K.J. CHAN #####################
            if "safe_cand_as_of" in gen_results.gen_metadata.keys():
                # print("untransforming safe candidates...")
                if bool(gen_results.gen_metadata["safe_cand_as_of"]):
                    gen_results.gen_metadata["safe_cand_as_of"] = t.untransform_observation_features(
                        gen_results.gen_metadata["safe_cand_as_of"]
                    )
            ###################### END EDITS BY K.J. CHAN ######################
        ...
        if not self._fit_out_of_design:
            ...
            ##################### BEGIN EDITS BY K.J. CHAN #####################
            if "safe_cand_as_of" in gen_results.gen_metadata.keys():
                # print("clamping safe candidates...")
                if bool(gen_results.gen_metadata["safe_cand_as_of"]):
                    gen_results.gen_metadata["safe_cand_as_of"] = clamp_observation_features(
                        gen_results.gen_metadata["safe_cand_as_of"], orig_search_space
                    )
            ###################### END EDITS BY K.J. CHAN ######################
